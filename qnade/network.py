...
import qnade.autograd_hacks as ah
    ...
    def forward(self, N_samples=None, x=None, requires_grad=True):
        ...
        for d in range(self.D):
            ...
            if need_samples == True:
                ...
                m = torch.where(
                    m == 0, torch.tensor(-1).to(device), torch.tensor(1).to(device)
                )
            ...
            if requires_grad == True:
                ...
                for output in range(len(self.evals)):
                    ...
                    ah.compute_grad1(self.model)
                    ah.clear_backprops(self.model)
